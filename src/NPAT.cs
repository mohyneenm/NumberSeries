...
{
    // RULE 0: Assume that only the last item in the given series is to be predicted, not intermediate items for now.
    ...
    // RULE 8: Should LTR operartions be given higher precedence than RTL operations? Given {2, 3, 3, 4, 1, 5, 7, -2, 8} => {(2, 3, 3), (4, 1, 5), (7, -2, 8), (3, 2, x)} => a+b=5, c+b=6, which pattern has higher precedence?
